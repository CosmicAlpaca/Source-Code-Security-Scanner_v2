...
**Nhiều engine (opt-in):** ngoài Semgrep, radar tích hợp registry các scanner khác — bật bằng `--engine` (lặp lại được) hoặc `--all-tools`:

```bash
radar engines                                   # liệt kê engine + runtime sẵn có trên máy
radar scan . --engine semgrep --engine gitleaks # chạy nhiều engine
radar scan . --all-tools                        # chạy tất cả engine mặc định
```

| Engine | Bắt gì | Runtime |
|---|---|---|
| `semgrep` *(mặc định)* | SAST: preset OWASP + 50 bundled rules (30+ ngôn ngữ) | native `semgrep` hoặc Docker |
| `gitleaks` | secrets — API key, token, credential | vendored (đi kèm package) |
| `bandit` | SAST Python chuyên sâu | `pip install bandit` |
| `trivy` | CVE dependency (SCA) + IaC misconfig + secrets | Docker |

`radar engines` in bảng trạng thái runtime (`native` / `vendored` / `docker` / `unavailable`) để biết engine nào chạy được ngay.
...
## Phần 3.10 — `radar analyze`: quét nhanh repo GitHub bất kỳ

Một lệnh duy nhất: **clone repo từ URL → scan → AI triage → xuất dashboard HTML**. Tiện khi muốn soi nhanh một repo ngoài mà không cần clone tay.

```bash
radar analyze https://github.com/OWASP/NodeGoat        # clone + scan + triage + dashboard
radar analyze <url> --branch develop                   # chọn nhánh
radar analyze <url> --function loginHandler            # kèm impact trace cho 1 hàm
radar analyze <url> --out report.html                  # chọn đường dẫn output
```

Repo được clone vào thư mục tạm *ngoài* repo của bạn (zero-footprint). AI triage cần `OPENAI_API_KEY`; thiếu key thì bỏ qua phần verdict, vẫn ra dashboard có Risk Ranking offline.
...
> `fetch-depth: 0` ở bước checkout là **bắt buộc** để impact diff được với nhánh base. Findings **không block merge** trừ khi bạn đặt `fail-on`.

> **Muốn thử CI end-to-end trên một fork?** Xem tutorial từng bước (fork → thêm workflow → mở PR → đọc SARIF/Security tab + PR comment): [docs/test-ci-on-forked-repo.md](docs/test-ci-on-forked-repo.md).

---

## Demo nhanh

`demo/app/` là Express app **cố ý chứa lỗ hổng** (SQLi, command injection, hardcoded secret) — không deploy.
...
| CLI | [Click](https://click.palletsprojects.com) | 11 lệnh: `scan / build / impact / report / triage / graph / history / watch / analyze / serve / engines` |
...
| Testing | pytest + Semgrep `--test` | 406 unit tests; rule fixtures với `// ruleid:` / `ok:` |
...
[PRD](docs/security-radar-prd.md) · [Kiến trúc](docs/system-architecture.md) · [Changelog](docs/project-changelog.md) · [Roadmap](docs/development-roadmap.md) · [Test CI trên fork](docs/test-ci-on-forked-repo.md)
